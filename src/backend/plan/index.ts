...
export class Plan {
    ...
    private resetMonthlyVotesIfNeeded(agentId: string): void {
        const now = new Date();
    ...
    getAgentPlanInfo(agentId: string): AgentPlan {
        this.resetMonthlyVotesIfNeeded(agentId);
    ...
    trackVote(agentId: string, voterId: string): void {
        ...
        if (agentPlan.plan === 'free' && agentPlan.voteCount >= limits.maxVotesPerMonth) {
            throw new Error(`Free plan limited to ${limits.maxVotesPerMonth} votes per month. Upgrade to premium.`);
        }
    ...
    checkCreatePollLimits(agentId: string, options: any[], tags: any[]): void {
        ...
        if (agentPolls.length >= limits.maxPolls) {
            throw new Error(`Free plan limited to ${limits.maxPolls} polls. Upgrade to premium for unlimited polls.`);
        }
