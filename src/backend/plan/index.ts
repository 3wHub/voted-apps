...
export class Plan {
    ...
    public validateCreatePollLimits(agentId: string, options: any[], tags: any[]): void {
        ...
        if (options.length > limits.maxOptions) {
            throw new Error(`Maximum of ${limits.maxOptions} options allowed for ${agentPlan.plan} plan`);
        }

        if (tags.length > limits.maxTags) {
            throw new Error(`Maximum of ${limits.maxTags} tags allowed for ${agentPlan.plan} plan`);
        }
    }
